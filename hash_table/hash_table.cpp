...
uint64_t hash_table_hash_function(char *str)
{
	uint64_t i = 0;

	for(int64_t j = 0; str[j]; ++j) {
		i += str[j];
	}
...
HashTable *hash_table_create(size_t input_table_size)
{
	...
	for(uint32_t i = 0; i < hash_table->table_size; ++i) {
...
{
	uint64_t index = hash_table_hash_function(input_key);
...
void hash_table_handle_collision(HashTable *hash_table, uint64_t index, Item *input_item)
{
	List *head = hash_table->overflow_buckets[index];
...
{
	int32_t index = hash_table_hash_function(item->key);
